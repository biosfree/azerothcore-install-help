...
local Config_senderGUID = 10667    -- (10667) GUID/ID of the Player/Creature. If Config_preventReturn = true then you need to put Creature ID. If it's false Player GUID. 0 = No sender aka "From: Unknown".
...
local Config_announce_on_login = false   -- announce on login
local Config_announcement = "This server is running the |cFF00B0E8LevelUp Reward|r lua script."
...
local Config_mailSubject2A = "Награда от Хроми за "
local Config_mailSubject2B = " уровень"
local Config_mailText2A = " \n\nЯ заглянула в ваше будущее и… еще не пришло время посвящать вас в эти знания, позже мы поговорим об этом.\n\nСейчас я хочу обсудить тот поразительный факт, что вы "
local Config_mailText2B = " искатель приключений "
local Config_mailText2C = ", достигнувший "
local Config_mailText2D = " уровня мастерства и тем самым обратившим на себя моё внимание.\n\nВ знак моей признательности я прошу вас принять эту небольшую награду, которая поможет вам двигаться вперед!\n\nС уважением,\nХроми"

-- Name of Eluna dB scheme
local Config_customDbName = 'acore_world';
...
local function BroadcastLoginAnnouncement(event, player)
    if Config_announce_on_login then
        player:SendBroadcastMessage(Config_announcement)
    end
end
    ...
    if oldLevel ~= nil and player:GetGMRank() <= Config_maxGMRank then
        if Config_mailText == 1 then
            if Config_ItemId[oldLevel + 1] ~= nil then
                ...
                SendMail(Config_mailSubject1, "Привет "..playerName..Config_mailText1, playerGUID, Config_senderGUID, Config_mailStationery, 0, Config_Gold[oldLevel + 1],0,Config_ItemId[oldLevel + 1], itemAmount)
            ...
            elseif Config_Gold[oldLevel + 1] ~= nil then
                local playerName = player:GetName()
                local playerGUID = tostring(player:GetGUID())
                SendMail(Config_mailSubject1, "Привет "..playerName..Config_mailText1, playerGUID, Config_senderGUID, Config_mailStationery, 0, Config_Gold[oldLevel + 1])
        ...
        if string.sub(playerCounterStr, -1) ==  "1" or  string.sub(playerCounterStr, -1) == "4" or string.sub(playerCounterStr, -1) == "5" or string.sub(playerCounterStr, -1) == "10"  then
            playerCounterStr = playerCounterStr.."ый"
        elseif string.sub(playerCounterStr, -1) == "4" or string.sub(playerCounterStr, -1) == "5" or string.sub(playerCounterStr, -1) == "10"  then
            playerCounterStr = "уже "..playerCounterStr.."ый"
        elseif string.sub(playerCounterStr, -1) == "2" or string.sub(playerCounterStr, -1) == "6" or string.sub(playerCounterStr, -1) == "7" or string.sub(playerCounterStr, -1) == "8" then
            playerCounterStr = "уже "..playerCounterStr.."ой"
        elseif string.sub(playerCounterStr, -1) == "3" then
            playerCounterStr = "уже "..playerCounterStr.."ий"
        else
            playerCounterStr = "уже "..playerCounterStr..""
        end

        currentLevelStr = tostring(currentLevel)
        if string.sub(currentLevelStr, -1) == (1)  then
            currentLevelStr = currentLevelStr.."ый"
        elseif string.sub(currentLevelStr, -1) == (2) then
            currentLevelStr = currentLevelStr.."ой"
        elseif string.sub(currentLevelStr, -1) == (3) then
            currentLevelStr = currentLevelStr.."ий"
        else
            currentLevelStr = currentLevelStr..""
        end
				
        local FactionStr = {}
        local playerTeam = player:GetTeam()
        if playerTeam == 0 then -- Alliance
				    FactionStr = "в Альянсе"
        ...
        if Config_mailText == 2 then
            if Config_ItemId[oldLevel + 1] ~= nil and Config_mailText == 2 then
                ...
                if Config_Gold[oldLevel + 1] == nil then
                    Config_Gold[oldLevel + 1] = 0
								...
								local ChatMess = "|cFF00B0E8"..playerName.." получил награду от Хроми за "..currentLevelStr..Config_mailSubject2B.."|r"
								player:SendBroadcastMessage(ChatMess)
                SendMail(Config_mailSubject2A..currentLevelStr..Config_mailSubject2B, "Род бронзовых драконов приветствую вас "..playerName..Config_mailText2A..playerCounterStr..Config_mailText2B..FactionStr..Config_mailText2C..currentLevelStr..Config_mailText2D, playerGUID, Config_senderGUID, Config_mailStationery, 0, Config_Gold[oldLevel + 1],0,Config_ItemId[oldLevel + 1], itemAmount)
                print("LevelUpReward has granted "..Config_Gold[oldLevel + 1].." copper and "..itemAmount.." of item "..Config_ItemId[oldLevel + 1].." to character "..playerName.." with guid "..playerGUID..".")
                PreventReturn(playerGUID)
                playerName = nil
                playerGUID = nil
            ...
            elseif Config_Gold[oldLevel + 1] ~= nil and Config_mailText == 2 then
                ...
                local playerGUID = tostring(player:GetGUID())
								local ChatMess = "|cFF00B0E8"..playerName.." получил награду от Хроми за "..currentLevelStr..Config_mailSubject2B.."|r"
								player:SendBroadcastMessage(ChatMess)
                SendMail(Config_mailSubject2A..currentLevelStr..Config_mailSubject2B, "Род бронзовых драконов приветствую вас "..playerName..Config_mailText2A..playerCounterStr..Config_mailText2B..FactionStr..Config_mailText2C..currentLevelStr..Config_mailText2D, playerGUID, Config_senderGUID, Config_mailStationery, 0, Config_Gold[oldLevel + 1])
                print("LevelUpReward has granted "..Config_Gold[oldLevel + 1].." copper to character "..playerName.." with guid "..playerGUID..".")
                PreventReturn(playerGUID)
                playerName = nil
                playerGUID = nil
...
RegisterPlayerEvent(3, BroadcastLoginAnnouncement)
